		...
		<div class="row">
			<div class="col-xs-12 a-left">
				<ul class="breadcrumb" itemscope itemtype="https://data-vocabulary.org/Breadcrumb">					
					...
					<li><strong ><span itemprop="title"> Sản phẩm - {{$detail}}</span></strong></li>
						...
						<section class="products-view products-view-grid collection_reponsive list_hover_pro">
									...
									<div class="item_product_main item_product_main_relative">
										<div class="product-item grid product-box product-item-main clearfix">
											<div class="product-thumbnail">
												...
												<a class="image_thumb" href="{{route('user.product_detail',['slug' => $pro_ct->slug])}}" title="{{$pro_ct->name}}">
													<img src="public/product_upload/{{$pro_ct->image}}?1548750789768" data-lazyload="public/product_upload/{{$pro_ct->image}}?v=1545215353747" alt="{{$pro_ct->name}}">
												</a>
											...
											<div class="product-detail product-info">
												...
												<h3 class="product-title"><a href="{{route('user.product_detail',['slug' => $pro_ct->slug])}}" title="{{$pro_ct->name}}">{{$pro_ct->name}}</a></h3>

												<div class="blockprice ">
													@if($pro_ct->price_sale != 0)
														<div class="product-item-price price-box">
															<span class="price-style">
																<span class="old-price">{{number_format($pro_ct->price)}} VND</span>
															</span>

															<span class="price-style">
																<span class="special-price">{{number_format($pro_ct->price_sale)}} VND</span>
															</span>	
														</div>
														@else()
														<div class="product-item-price price-box">
															...
															<span class="price-style">
																<span class="special-price">{{number_format($pro_ct->price)}} VND</span>
															</span>	
														</div>
														@endif()
												</div>
											...
											<div class="button-view product-action clearfix">
													<div class="add-cart btn-buy button">
														<a href="{{route('user.product_detail',['slug' => $pro_ct->slug])}}" class="btn-buy btn-cart button_35 left-to disabled" title="Liên hệ">
							...
							<div class="text-center">
								<nav class="clearfix relative nav_pagi f-left w_100">
									{{$pro_cts->appends(request()->only('search_pros'))->links()}}
								</nav>
							</div>
